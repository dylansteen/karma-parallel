...
function handleBrowserRegister(log, config, browser) {
  ...
  const freeBrowsers = config.freeBrowserIndexes[browser.name];
  if (freeBrowsers && freeBrowsers.length) {
    config.shardIndexMap[browser.id] = freeBrowsers.pop();
    return;
  }
...
module.exports = function(/* config */ fullConfig, emitter, logger) {
  ...
  const handleBrowserUnregister = (browser) => {
    const {
      id,
      lastResult: { disconnected }
    } = browser;
    const { shardIndexMap, freeBrowserIndexes } = config;
    const browserShardId = shardIndexMap[id];
    const freeBrowsers = freeBrowserIndexes[browser.name] || [];
    if (disconnected && freeBrowsers.indexOf(browserShardId) < 0) {
      if (!freeBrowserIndexes[browser.name])  {
        freeBrowserIndexes[browser.name] = freeBrowsers;
      }
      freeBrowsers.push(browserShardId);
    }
  };
  ...
  emitter.on('browser_error', function(browser) {
    handleBrowserUnregister(browser);
  });

  emitter.on('browser_complete', function(browser) {
    handleBrowserUnregister(browser);
  });
};
